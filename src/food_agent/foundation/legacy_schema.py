...
restaurants = Table(
    ...
    Column("provider_refs", JSONB, server_default=text("'{}'::jsonb")),
    ...
    Column("coordinate_system", String(32)),
    Column("geo", JSONB, server_default=text("'{}'::jsonb")),
    Column("recommended_dishes", JSONB, server_default=text("'[]'::jsonb")),
    Column("promotions", JSONB, server_default=text("'[]'::jsonb")),
    Column("profile_metadata", JSONB, server_default=text("'{}'::jsonb")),
    Column("review_completeness", JSONB, server_default=text("'{}'::jsonb")),
    Column("profile_gaps", JSONB, server_default=text("'[]'::jsonb")),
    Column("source_payload", JSONB),
    Column("source_updated_at", DateTime(timezone=True)),
    Column("profile_fetched_at", DateTime(timezone=True)),
    Column("profile_refresh_status", String(20)),
    Column("created_at", DateTime(timezone=True), server_default=text("CURRENT_TIMESTAMP")),
    Column("updated_at", DateTime(timezone=True), server_default=text("CURRENT_TIMESTAMP")),
)
