...
def build_reliable_research_worker(
    ...
    *,
    ...
    from food_agent.orchestrator.reliable_task import (
        ReliableTaskConfig,
...
def build_refresh_worker(
    ...
    *,
    ...
    from food_agent.foundation import TemporalTaskQueues, build_temporal_refresh_worker

    queues = task_queues if isinstance(task_queues, TemporalTaskQueues) else TemporalTaskQueues()
    if not workflows:
        from food_agent.orchestrator import TemporalRefreshWorkflow
...
def build_media_worker(
    ...
    *,
    ...
    from food_agent.foundation import TemporalTaskQueues, build_temporal_media_worker

    queues = task_queues if isinstance(task_queues, TemporalTaskQueues) else TemporalTaskQueues()
    if not workflows:
        from food_agent.orchestrator import TemporalMediaWorkflow
...
def build_composition_root(
    *,
    ...
    from food_agent.composition.modular_bindings import CapabilityMode
    ...
    from food_agent.domain_packs.food import load_food_contract_resources
    from food_agent.domain_packs.food.adaptive_pack import FoodAdaptivePack
    from food_agent.domain_packs.food.pack import FoodBehavior
    from food_agent.foundation import (
        Boto3ObjectStore,
    ...
    from food_agent.orchestrator.core import XHSFoodOrchestrator
    ...
    from food_agent.research.adaptive import AdaptiveCritic, AdaptivePlanner
    from food_agent.research.adaptive.food_workflow import AdaptiveFoodResearchWorkflow
    from food_agent.services import LLMService, get_session_manager, get_user_storage_service
    ...
    if account_service_registry is None and (account_config_explicit or ambient_target_bindings_enabled):
        from food_agent.composition.account_services import build_account_service_registry
    ...
    if policy_explicit or ambient_target_bindings_enabled:
        ...
        from food_agent.contracts import AgentToolPolicy
    ...
    if modular_plan.observability_enabled:
        if modular_overrides.observation_port is None:
            ...
            from food_agent.composition.adapters import build_observation_exporter
        ...
        if modular_overrides.evaluation_port is None:
            from food_agent.composition.adapters import build_evaluation_port
    ...
    async def legacy_event_bus() -> LegacyEventBusAdapter:
        from food_agent.events.bus import get_event_bus
    ...
    if memory_bindings_enabled:
        from food_agent.composition.adapters import MemoryAuthorityWriter, MemoryOutboxProjector
