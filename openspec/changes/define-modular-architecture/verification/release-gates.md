...
| Browser | Chromium desktop/mobile | Chromium local matrix passed at `1280x900` and `390x844`; Firefox/WebKit binaries are unavailable in this environment |
...
| 14.4 browser matrix | **PARTIAL** | `uv run --frozen python scripts/qualification_frontend_browser_matrix.py` with `FOOD_AGENT_BROWSER_MATRIX_BROWSERS=chromium` passed search, first-503 SSE reconnect, favorites, history, and profile FAQ at desktop `1280x900` and mobile `390x844`. Firefox and WebKit remain unexecuted because their Playwright binaries could not be installed in this environment; the full approved browser matrix is not qualified. |
...
| 14.10 Temporal/operator gate | **PARTIAL** | Nine isolated SDK qualification tests pass (`9 passed in 70.17s`), including determinism, model/tool Activities, retry, cancellation, patched replay, duplicate Workflow admission and duplicate commit delivery; application cancellation and PG/Temporal reconciliation evidence pass. Production rollout, multi-worker smoke and operator recovery remain pending. |
