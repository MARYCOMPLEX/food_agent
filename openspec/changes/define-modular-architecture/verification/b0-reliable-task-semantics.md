...
| 8.7 | Failure-injection and differential suite | PARTIAL | Fifteen combined live tests pass: eight Temporal SDK tests, PostgreSQL authority, Redis replay/retention, application commit/reconcile, HTTP/SSE retained/expired cursor cases, local OS process-crash/resume, and live PG/Temporal crash-after-commit recovery. Offline failed-receipt ordering, same-run terminal competition, commit failure, reconciliation, late-run, and Redis replay contracts also pass. Duplicate Activity against a live worker, target service restart, and browser differential cases remain pending. |
...
uv run --frozen pytest -q -m "not live" -ra --durations=0
  # 868 passed, 22 deselected, 2 warnings in 53.70s
...
`2026-08-24`. The combined live command records fifteen isolated tests in
`112.80s`; retry recovery and exhaustion share one test function. The local
...
| Live application binding count/duration | `1 passed in 12.63s`; live HTTP/SSE `1 passed in 7.99s`; local process-crash `1 passed in 19.78s`; PG/Temporal crash-after-commit `1 passed in 44.28s`; current combined Temporal/PostgreSQL/Redis/application/process gate `15 passed in 112.80s` |
| Full non-live count/duration | `868 passed, 22 deselected, 2 warnings in 53.70s` |
| `uv lock --check` | `pass` |
