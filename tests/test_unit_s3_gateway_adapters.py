...
def _xhs_connector(
    ...
    *,
    note_provider: FakeProvider | None = None,
    batch_provider: FakeProvider | None = None,
) -> XHSSourceConnector:
    return XHSSourceConnector(
        search_provider=search_provider,
        note_provider=note_provider
        or FakeProvider("xhs_note", ProviderResult(success=True, data={"notes": []})),
        batch_provider=batch_provider
        or FakeProvider("xhs_batch", ProviderResult(success=True, data={"results": {}})),
        clock=lambda: NOW,
    )
    ...
    return SourceLocator.model_validate(
        {
            ...
            "retention": {
                "retention_class": "source-default",
...
async def test_xhs_success_and_true_empty_remain_distinguishable() -> None:
        ...
        ProviderResult(
            ...
            data={
                "notes": [
                    {
                        "id": "note-1",
                        "title": "Zigong local food",
                        "desc": "A useful note",
                        "url": "https://www.xiaohongshu.com/explore/note-1?token=secret",
                    }
                ]
            },
        ),
    )
    success_batch = await _xhs_connector(success_provider).search(_collect_request())

    assert classify_batch(success_batch).kind is SourceOutcomeKind.SUCCESS
    assert [item.external_id for item in success_batch.documents] == ["note-1"]
    assert str(success_batch.documents[0].canonical_url) == (
        "https://www.xiaohongshu.com/explore/note-1"
    )
    assert success_batch.errors == ()
    assert project_legacy_xhs(classify_batch(success_batch)) is LegacySourceProjection.CONTINUE
    assert success_provider.calls == [
        {
            "keyword": "cn.sc.zigong restaurant",
            "count": 10,
    ...
    empty_provider = FakeProvider("xhs_search", ProviderResult(success=True, data={"notes": []}))
    empty_batch = await _xhs_connector(empty_provider).search(_collect_request())

    assert classify_batch(empty_batch).kind is SourceOutcomeKind.EMPTY
    assert empty_batch.documents == ()
    assert empty_batch.errors == ()
    assert project_legacy_xhs(classify_batch(empty_batch)) is LegacySourceProjection.TERMINAL_ERROR


async def test_source_ready_query_is_rendered_before_connector_dispatch() -> None:
    xhs_provider = FakeProvider("xhs_search", ProviderResult(success=True, data={"notes": []}))
    await _xhs_connector(xhs_provider).search(
        _collect_request(source_text="自贡 美食", source_locality="自贡")
    )
    amap = FakeAmapClient({"count": 0, "pois": []})
    await AmapPlaceSourceConnector(amap, clock=lambda: NOW).search(
        _collect_request(
            "amap",
            source_text="自贡 餐厅",
            source_locality="自贡",
    ...
    assert xhs_provider.calls[0]["keyword"] == "自贡 美食"
    assert amap.calls == [{"keywords": "自贡 餐厅", "city": "自贡", "types": "050000"}]


@pytest.mark.parametrize(
    "projection_update",
    ({"source_id": "amap"}, {"language": "en"}),
    ids=("source-mismatch", "language-mismatch"),
)
async def test_invalid_explicit_source_projection_is_rejected_before_provider_call(
    projection_update: dict[str, str],
) -> None:
    request = _collect_request(source_text="自贡 美食", source_locality="自贡")
    projection = request.source_queries["xhs"].model_copy(update=projection_update)
    invalid_request = request.model_copy(update={"source_queries": {"xhs": projection}})
    provider = FakeProvider("xhs_search", ProviderResult(success=True, data={"notes": []}))

    with pytest.raises(ValueError, match="source query projection"):
        await _xhs_connector(provider).search(invalid_request)

    assert provider.calls == []


async def test_xhs_note_and_batch_keep_legacy_names_arguments_and_payload_shapes() -> None:
    note_provider = FakeProvider(
        "xhs_note",
        ProviderResult(
            success=True,
            data={
                "id": "note-1",
                "title": "detail",
                "full_desc": "full detail text",
                "comments": [{"id": "comment-1", "content": "useful"}],
            },
        ),
    )
    batch_provider = FakeProvider(
        ...
        ProviderResult(
            ...
            data={
                "status": "success",
                "results": {
                    "topic-a": [{"id": "note-a", "title": "A"}],
                    "topic-b": [{"id": "note-b", "title": "B"}],
                },
            },
        ),
    )
    connector = _xhs_connector(
        FakeProvider("xhs_search", ProviderResult(success=True, data={"notes": []})),
        note_provider=note_provider,
        batch_provider=batch_provider,
    )

    batch = await connector.batch_search(
        _collect_request(), ("topic-a", "topic-b"), notes_per_topic=3
    )
    document = await connector.fetch_document(_source_locator())
    comments = await connector.fetch_comments(_source_locator())

    assert [item.external_id for item in batch.documents] == ["note-a", "note-b"]
    assert batch_provider.calls == [{"topics": ["topic-a", "topic-b"], "notes_per_topic": 3}]
    assert document.external_id == "note-1"
    assert document.text == "full detail text"
    assert [item.external_id for item in comments.comments] == ["comment-1"]
    assert note_provider.calls == [
        {"note_id": "note-1", "max_comments": 30},
        {"note_id": "note-1", "max_comments": 30},
    ]

    with pytest.raises(ValueError, match="tool names or order changed"):
        XHSSourceConnector(
            search_provider=FakeProvider("xhs_note", None),
            note_provider=FakeProvider("xhs_search", None),
            batch_provider=FakeProvider("xhs_batch", None),
        )


@pytest.mark.parametrize(
    ("provider_result", "category", "retryable"),
    [
        (
            ProviderResult(success=False, error_code="SOURCE_TIMEOUT"),
        ...
        (
            ProviderResult(success=False, error_code="MALFORMED_RESPONSE"),
...
async def test_xhs_provider_failures_keep_source_taxonomy(
    provider_result: ProviderResult,
    category: ErrorCategory,
    retryable: bool,
) -> None:
    batch = await _xhs_connector(FakeProvider("xhs_search", provider_result)).search(
        _collect_request()
    )
    ...
    assert project_legacy_xhs(outcome) is LegacySourceProjection.TERMINAL_ERROR


async def test_xhs_malformed_envelope_and_provider_exception_have_distinct_scope() -> None:
    malformed_batch = await _xhs_connector(
        FakeProvider("xhs_search", {"status": "success", "notes": []})
    ).search(_collect_request())
    malformed = malformed_batch.errors[0]

    assert classify_batch(malformed_batch).kind is SourceOutcomeKind.FAILURE
    assert malformed.code == "XHS_RESULT_ENVELOPE_MALFORMED"
    assert malformed.category is ErrorCategory.MALFORMED_RESPONSE
    assert malformed.scope is ErrorScope.SOURCE
    assert malformed.retryable is False

    exception_batch = await _xhs_connector(
        FakeProvider("xhs_search", result=None, error=RuntimeError("provider exploded"))
    ).search(_collect_request())
    exception = exception_batch.errors[0]

    assert classify_batch(exception_batch).kind is SourceOutcomeKind.FAILURE
    assert exception.code == "PROVIDER_INTERNAL"
    assert exception.category is ErrorCategory.INTERNAL
    assert exception.scope is ErrorScope.PROVIDER
    assert exception.retryable is False

    class MissingFailureFields:
        success = False

    missing_fields_batch = await _xhs_connector(
        FakeProvider("xhs_search", MissingFailureFields())
    ).search(_collect_request())
    missing_fields = missing_fields_batch.errors[0]
    assert missing_fields.code == "XHS_RESULT_ENVELOPE_MALFORMED"
    assert missing_fields.category is ErrorCategory.MALFORMED_RESPONSE
    assert missing_fields.scope is ErrorScope.SOURCE


async def test_xhs_duplicate_document_ids_follow_legacy_first_item_wins() -> None:
    batch = await _xhs_connector(
        FakeProvider(
            "xhs_search",
            ProviderResult(
                success=True,
                data={
                    "notes": [
                        {"id": "same-note", "title": "first"},
                        {"id": "same-note", "title": "second"},
                    ]
                },
            ),
        )
    ...
    assert [document.external_id for document in batch.documents] == ["same-note"]
    assert batch.documents[0].title == "first"
    assert batch.errors == ()


async def test_xhs_partial_batch_keeps_surviving_documents_and_errors() -> None:
    ...
    assert [item.external_id for item in batch.documents] == ["note-1"]
    assert [error.code for error in outcome.errors] == ["XHS_ITEM_ID_MISSING"]
    assert project_legacy_xhs(outcome) is LegacySourceProjection.CONTINUE


async def test_xhs_hang_has_no_implicit_deadline_but_propagates_cancellation() -> None:
    provider = HangingProvider("xhs_search")
    task = asyncio.create_task(_xhs_connector(provider).search(_collect_request()))
